...
type HTMLCanvasElement(r : JSObject) =
    ...
    member x.GetWebGLContext(att : WebGLContextAttributes) =
        r.Invoke("getContext", "webgl2", js att) |> unbox<JSObject> |> WebGLRenderingContext
...
let main _argv =
    ...
    let gl = c.GetWebGLContext(WebGLContextAttributes(Desynchronized = false))
    ...
    let shaders = gl.GetAttachedShaders(p) 
    ...
    for e in gl.GetSupportedExtensions() do
        printfn "%A" e


    printfn "%A" (gl.GetContextAttributes())
