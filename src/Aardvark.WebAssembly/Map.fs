...
[<DebuggerTypeProxy("MapExt.MapExtDebugView`2")>]
[<DebuggerDisplay("Count = {Count}")>]
[<CompiledName("FSharpMapExt`2")>]
[<Sealed>]
type MapExt< [<EqualityConditionalOn>] 'Key, [<EqualityConditionalOn;ComparisonConditionalOn>] 'Value when 'Key : comparison> private(comparer : IComparer<'Key>, root : MapNode<'Key, 'Value>) =
        
    static let defaultComparer = LanguagePrimitives.FastGenericComparer<'Key>
    static let empty = MapExt<'Key, 'Value>(defaultComparer, MapEmpty.Instance)
    ...
    static member FromArray (elements : array<'Key * 'Value>) =
        ...
        match elements.Length with
        | 0 -> 
            MapExt(cmp, MapEmpty.Instance)
        | 1 ->
            let (k,v) = elements.[0]
            MapExt(cmp, MapLeaf(k, v))
        | 2 -> 
            let (k0,v0) = elements.[0]
            let (k1,v1) = elements.[1]
            let c = cmp.Compare(k0, k1)
            if c > 0 then MapExt(cmp, MapInner(MapEmpty.Instance, k1, v1, MapLeaf(k0, v0)))
            elif c < 0 then MapExt(cmp, MapInner(MapLeaf(k0, v0), k1, v1, MapEmpty.Instance))
            else MapExt(cmp, MapLeaf(k1, v1))
        | 3 ->
            let (k0,v0) = elements.[0]
            let (k1,v1) = elements.[1]
            let (k2,v2) = elements.[2]
            MapExt(cmp, MapLeaf(k0, v0).AddInPlace(cmp, k1, v1).AddInPlace(cmp, k2, v2))
        | 4 ->
            let (k0,v0) = elements.[0]
            let (k1,v1) = elements.[1]
            let (k2,v2) = elements.[2]
            let (k3,v3) = elements.[3]
            MapExt(cmp, MapLeaf(k0, v0).AddInPlace(cmp, k1, v1).AddInPlace(cmp, k2, v2).AddInPlace(cmp, k3, v3))
        | 5 ->
            let (k0,v0) = elements.[0]
            let (k1,v1) = elements.[1]
            let (k2,v2) = elements.[2]
            ...
            MapExt(cmp, MapLeaf(k0, v0).AddInPlace(cmp, k1, v1).AddInPlace(cmp, k2, v2).AddInPlace(cmp, k3, v3).AddInPlace(cmp, k4, v4))
        | _ ->
            let struct(arr, cnt) = Sorting.mergeSortHandleDuplicates false cmp elements elements.Length
            MapExt.CreateTree(cmp, arr, cnt)
        ...
        | ((k0, v0) as t0) :: rest ->
            ...
            | ((k1, v1) as t1) :: rest ->
                ...
                | [] ->
                    // cnt = 2
                    let c = cmp.Compare(k0, k1)
                    if c < 0 then MapExt(cmp, MapInner(MapLeaf(k0, v0), k1, v1, MapEmpty.Instance))
                    elif c > 0 then MapExt(cmp, MapInner(MapEmpty.Instance, k1, v1, MapLeaf(k0, v0)))
                    else MapExt(cmp, MapLeaf(k1, v1))
                | ((k2, v2) as t2) :: rest ->
                    // cnt >= 3
                    match rest with
                    ...
                    | ((k3, v3) as t3) :: rest ->
                        ...
                        | ((k4, v4) as t4) :: rest ->
                            ...
                            | t5 :: rest ->
                                ...
                                MapExt.CreateTree(cmp, arr1, cnt1)
    ...
    static member FromSeq (elements : seq<'Key * 'Value>) =
        match elements with
        | :? array<'Key * 'Value> as e -> MapExt.FromArray e
        | :? list<'Key * 'Value> as e -> MapExt.FromList e
        | _ ->
            let cmp = defaultComparer
    ...
    static member FromSeqV (elements : seq<struct('Key * 'Value)>) =
        match elements with
        | :? array<struct('Key * 'Value)> as e -> MapExt.FromArrayV e
        | :? list<struct('Key * 'Value)> as e -> MapExt.FromListV e
        | _ ->
            let cmp = defaultComparer
            use e = elements.GetEnumerator()
            if e.MoveNext() then
    ...
    static member Union(l : MapExt<'Key, 'Value>, r : MapExt<'Key, 'Value>) =
        ...
        let cmp = defaultComparer
        MapExt(cmp, union cmp l.Root r.Root)
    ...
    static member UnionWith(l : MapExt<'Key, 'Value>, r : MapExt<'Key, 'Value>, resolve : 'Key -> 'Value -> 'Value -> 'Value) =
        ...
        let cmp = defaultComparer
        MapExt(cmp, union cmp resolve l.Root r.Root)
        
    member x.SplitV(key : 'Key) =
        let struct(l, r, s) = root.SplitV(comparer, key)
        struct(MapExt(comparer, l), s, MapExt(comparer, r))
        
    member x.Split(key : 'Key) =
        let struct(l, r, s) = root.SplitV(comparer, key)
        let s = match s with | ValueSome s -> Some s | _ -> None
    ...
    static member ComputeDelta<'T>(l : MapExt<'Key, 'Value>, r : MapExt<'Key, 'Value>, add : MapExt<'Key, 'Value> -> MapExt<'Key, 'T>, remove : MapExt<'Key, 'Value> -> MapExt<'Key, 'T>, update : 'Key -> 'Value -> 'Value -> voption<'T>) : MapExt<'Key, 'T> =
        
        let inline add (cmp : IComparer<_>) (a : MapNode<'Key, 'Value>) = 
            if a.Count > 0 then add(MapExt<'Key, 'Value>(cmp, a)).Root
            else MapEmpty.Instance

        let inline remove (cmp : IComparer<_>) (a : MapNode<'Key, 'Value>) = 
            if a.Count > 0 then remove(MapExt<'Key, 'Value>(cmp, a)).Root
            else MapEmpty.Instance
        ...
        MapExt(cmp, computeDelta cmp update l.Root r.Root)


    member x.Add(key : 'Key, value : 'Value) =
        MapExt(comparer, root.Add(comparer, key, value))
    ...
    member x.AddMatch(key : 'Key, value : 'Value) =
        ...
        MapExt(comparer, add comparer key value root)
            
    member x.Remove(key : 'Key) =
        MapExt(comparer, root.Remove(comparer, key))
    ...
    member x.RemoveMatch(key : 'Key) =
        ...
        MapExt(comparer, remove comparer key root)
    ...
    member x.Partition(predicate : 'Key -> 'Value -> bool) =
        ...
        MapExt.CreateTree(comparer, a0, i0), MapExt.CreateTree(comparer, a1, i1)
    ...
    member x.CopyTo(array : ('Key * 'Value)[], startIndex : int) =
        if startIndex < 0 || startIndex + x.Count > array.Length then raise <| System.IndexOutOfRangeException("MapExt.CopyTo")
        let rec copyTo (arr : array<_>) (index : int) (n : MapNode<_,_>) =
            match n with
            | :? MapInner<'Key, 'Value> as n ->
                let index = copyTo arr index n.Left
    ...
    member x.GetViewBetween(minInclusive : 'Key, maxInclusive : 'Key) = 
        MapExt(comparer, root.GetViewBetween(comparer, minInclusive, true, maxInclusive, true))
    ...
    member x.WithMin(minInclusive : 'Key) = 
        MapExt(comparer, root.WithMin(comparer, minInclusive, true))
        
    member x.WithMax(maxInclusive : 'Key) = 
        MapExt(comparer, root.WithMax(comparer, maxInclusive, true))
        
    member x.WithMinExclusive(minExclusive : 'Key) = 
        MapExt(comparer, root.WithMin(comparer, minExclusive, false))
        
    member x.WithMaxExclusive(maxExclusive : 'Key) = 
        MapExt(comparer, root.WithMax(comparer, maxExclusive, false))
    ...
    member x.Change(key : 'Key, f : option<'Value> -> option<'Value>) =
        MapExt(comparer, root.Change(comparer, key, f))
        
    member x.ChangeV(key : 'Key, update : voption<'Value> -> voption<'Value>) =
        MapExt(comparer, root.ChangeV(comparer, key, update))
    ...
    member x.CompareTo(other : MapExt<'Key, 'Value>) =
    ...
    override x.Equals o =
        match o with
        | :? MapExt<'Key, 'Value> as o -> equals comparer root o.Root
        | _ -> false
    ...
    member x.TryRemoveMin() =
        if root.Count = 0 then 
            ...
            let struct(k, v, rest) = root.UnsafeRemoveHeadV()
            ValueSome struct(struct(k,v), MapExt(comparer, rest))
            
    member x.TryRemoveMax() =
        if root.Count = 0 then 
            ValueNone
        else
            let struct(rest, k, v) = root.UnsafeRemoveTailV()
            ValueSome struct(struct(k,v), MapExt(comparer, rest))
    ...
    interface System.IComparable with
        member x.CompareTo o = 
            match o with
            | :? MapExt<'Key, 'Value> as o -> x.CompareTo o
            | _ -> raise <| ArgumentException()

    //interface System.IComparable<MapExt<'Key, 'Value>> with
    //    member x.CompareTo o = x.CompareTo o

    interface System.Collections.IEnumerable with
        member x.GetEnumerator() = new MapExtEnumerator<_,_>(root) :> _
    ...
    new(comparer : IComparer<'Key>) = 
        MapExt<'Key, 'Value>(comparer, MapEmpty.Instance)

    new(elements : seq<'Key * 'Value>) =
        let m = MapExt.FromSeq elements
        MapExt<'Key, 'Value>(m.Comparer, m.Root)

and [<NoComparison; NoEquality>] 
    MapExtEnumerator<'Key, 'Value> =
    struct
...
and internal MapExtDebugView<'Key, 'Value when 'Key : comparison> =

    [<DebuggerBrowsable(DebuggerBrowsableState.RootHidden)>]
    val mutable public Entries : KeyValuePairDebugFriendly<'Key, 'Value>[]

    new(m : MapExt<'Key, 'Value>) =
        {
...
module MapExt =
    ...
    let isEmpty (table : MapExt<'Key, 'Value>) = table.Count <= 0
    
    [<CompiledName("Count")>]
    let count (table : MapExt<'Key, 'Value>) = table.Count
    
    [<CompiledName("Add")>]
    let add (key : 'Key) (value : 'Value) (table : MapExt<'Key, 'Value>) = table.Add(key, value)
    
    [<CompiledName("Remove")>]
    let remove (key : 'Key) (table : MapExt<'Key, 'Value>) = table.Remove(key)

    [<CompiledName("Change")>]
    let change (key : 'Key) (f : option<'Value> -> option<'Value>) (table : MapExt<'Key, 'Value>) = table.Change(key, f)
    
    [<CompiledName("ChangeValue")>]
    let changeV (key : 'Key) (update : voption<'Value> -> voption<'Value>) (map : MapExt<'Key, 'Value>) = map.ChangeV(key, update)
        
    [<CompiledName("ReplaceRange")>]
    let replaceRange (minKey : 'Key) (maxKey : 'Key) (replace : voption<struct('Key * 'Value)> -> voption<struct('Key * 'Value)> -> struct(voption<'Value> * voption<'Value>)) (map : MapExt<'Key, 'Value>) = 
        map.ReplaceRange(minKey, maxKey, replace)

    [<CompiledName("TryFind")>]
    let tryFind (key : 'Key) (table : MapExt<'Key, 'Value>) = table.TryFind(key)
    
    [<CompiledName("TryFindValue")>]
    let tryFindV (key : 'Key) (map : MapExt<'Key, 'Value>) = map.TryFindV(key)
    
    [<CompiledName("ContainsKey")>]
    let containsKey (key : 'Key) (table : MapExt<'Key, 'Value>) = table.ContainsKey(key)
    
    [<CompiledName("Iterate")>]
    let iter (action : 'Key -> 'Value -> unit) (table : MapExt<'Key, 'Value>) = table.Iter(action)
    
    [<CompiledName("MapExt")>]
    let map (mapping : 'Key -> 'Value -> 'T) (table : MapExt<'Key, 'Value>) = table.Map(mapping)
    ...
    [<CompiledName("Filter")>]
    let filter (predicate : 'Key -> 'Value -> bool) (table : MapExt<'Key, 'Value>) = table.Filter(predicate)

    [<CompiledName("Exists")>]
    let exists (predicate : 'Key -> 'Value -> bool) (table : MapExt<'Key, 'Value>) = table.Exists(predicate)
    
    [<CompiledName("ForAll")>]
    let forall (predicate : 'Key -> 'Value -> bool) (table : MapExt<'Key, 'Value>) = table.Forall(predicate)

    [<CompiledName("Fold")>]
    let fold<'Key,'Value,'State when 'Key : comparison> (folder : 'State -> 'Key -> 'Value -> 'State) (state : 'State) (table : MapExt<'Key, 'Value>) = 
        table.Fold(folder, state)
    
    [<CompiledName("FoldBack")>]
    let foldBack (folder : 'Key -> 'Value -> 'State -> 'State) (table : MapExt<'Key, 'Value>) (state : 'State) = 
    ...
    let ofSeqV (values : seq<struct('Key * 'Value)>) = MapExt.FromSeqV values
    
    [<CompiledName("OfListValue")>]
    let ofListV (values : list<struct('Key * 'Value)>) = MapExt.FromListV values
    
    [<CompiledName("OfArrayValue")>]
    let ofArrayV (values : struct('Key * 'Value)[]) = MapExt.FromArrayV values

    [<CompiledName("ToSeq")>]
    let toSeq (table : MapExt<'Key, 'Value>) = table |> Seq.map (fun (KeyValue(k,v)) -> k, v)

    [<CompiledName("ToSeqValue")>]
    let toSeqV (map : MapExt<'Key, 'Value>) = map |> Seq.map (fun (KeyValue(k,v)) -> struct (k, v))

    [<CompiledName("ToList")>]
    let toList (table : MapExt<'Key, 'Value>) = table.ToList()
    
    [<CompiledName("ToListValue")>]
    let toListV (map : MapExt<'Key, 'Value>) = map.ToListV()
    
    [<CompiledName("ToArray")>]
    let toArray (table : MapExt<'Key, 'Value>) = table.ToArray()
    
    [<CompiledName("ToArrayValue")>]
    let toArrayV (map : MapExt<'Key, 'Value>) = map.ToArrayV()

    //[<CompiledName("Keys")>]
    //let keys (map : MapExt<'Key, 'Value>) = map.Keys()

    [<CompiledName("WithMin")>]
    let withMin (minInclusive : 'Key) (map : MapExt<'Key, 'Value>) = map.WithMin(minInclusive)
    
    [<CompiledName("WithMax")>]
    let withMax (maxInclusive : 'Key) (map : MapExt<'Key, 'Value>) = map.WithMax(maxInclusive)
    
    [<CompiledName("WithRange")>]
    let withRange (minInclusive : 'Key) (maxInclusive : 'Key) (map : MapExt<'Key, 'Value>) = map.GetViewBetween(minInclusive, maxInclusive)
    
    [<CompiledName("Union")>]
    let union (map1 : MapExt<'Key, 'Value>) (map2 : MapExt<'Key, 'Value>) = MapExt.Union(map1, map2)
    
    [<CompiledName("UnionMany")>]
    let unionMany (maps : #seq<MapExt<'Key, 'Value>>) =
    ...
    let unionWith (resolve : 'Key -> 'Value -> 'Value -> 'Value) (map1 : MapExt<'Key, 'Value>) (map2 : MapExt<'Key, 'Value>) = MapExt.UnionWith(map1, map2, resolve)
    
    [<CompiledName("TryMax")>]
    let tryMax (map : MapExt<'Key, 'Value>) = map.TryMaxKeyValue()

    [<CompiledName("TryMin")>]
    let tryMin (map : MapExt<'Key, 'Value>) = map.TryMinKeyValue()

    [<CompiledName("TryMaxValue")>]
    let tryMaxV (map : MapExt<'Key, 'Value>) = map.TryMaxKeyValueV()

    [<CompiledName("TryMinValue")>]
    let tryMinV (map : MapExt<'Key, 'Value>) = map.TryMinKeyValueV()
    
    [<CompiledName("TryAt")>]
    let tryAt (index : int) (map : MapExt<'Key, 'Value>) = map.TryAt index
    
    [<CompiledName("TryAtValue")>]
    let tryAtV (index : int) (map : MapExt<'Key, 'Value>) = 
        map.TryAtV index
        
    [<CompiledName("Find")>]
    let find (key : 'Key) (table : MapExt<'Key, 'Value>) =
        table.Find key
        
    [<CompiledName("FindKey")>]
    let findKey (predicate : 'Key -> 'Value -> bool) (table : MapExt<'Key, 'Value>) =
        table.FindKey(predicate)
        
    [<CompiledName("TryFindKey")>]
    let tryFindKey (predicate : 'Key -> 'Value -> bool) (table : MapExt<'Key, 'Value>) =
        table.TryFindKey(predicate)
        
    [<CompiledName("TryFindKeyValue")>]
    let tryFindKeyV (predicate : 'Key -> 'Value -> bool) (map : MapExt<'Key, 'Value>) =
        map.TryFindKeyV(predicate)

    [<CompiledName("TryPick")>]
    let tryPick (chooser : 'Key -> 'Value -> option<'T>) (table : MapExt<'Key, 'Value>) =
        table.TryPick(chooser)
        
    [<CompiledName("TryPickValue")>]
    let tryPickV (mapping : 'Key -> 'Value -> voption<'T>) (map : MapExt<'Key, 'Value>) =
        map.TryPickV(mapping)
        
    [<CompiledName("Pick")>]
    let pick (chooser : 'Key -> 'Value -> option<'T>) (table : MapExt<'Key, 'Value>) =
        table.Pick(chooser)

    [<CompiledName("PickValue")>]
    let pickV (mapping : 'Key -> 'Value -> voption<'T>) (map : MapExt<'Key, 'Value>) =
        map.PickV(mapping)
        
    [<CompiledName("Partition")>]
    let partition (predicate : 'Key -> 'Value -> bool) (table : MapExt<'Key, 'Value>) =
        table.Partition(predicate)
        
    [<CompiledName("Neighbours")>]
    let neighbours (key : 'Key) (table : MapExt<'Key, 'Value>) =
        table.Neighbours key

    [<CompiledName("NeighboursValue")>]
    let neighboursV (key : 'Key) (table : MapExt<'Key, 'Value>) =
        table.NeighboursV key
        
    [<CompiledName("NeighboursAt")>]
    let neighboursAt (index : int) (table : MapExt<'Key, 'Value>) =
        table.NeighboursAt index

    [<CompiledName("NeighboursAtValue")>]
    let neighboursAtV (index : int) (table : MapExt<'Key, 'Value>) =
